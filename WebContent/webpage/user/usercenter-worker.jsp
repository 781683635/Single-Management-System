			...
			<div class="col-sm-12">
				<div class="row">
					<h3 class="f-14 gradient-gray clear">
						<span class="pull-left pd-10 line-h-20 f-18">用户管理</span>
					</h3>
				</div>
				<br/>
				<!-- 操作用户 -->
				<form action="${pageContext.request.contextPath }/userqueryworker.do" method="post">
					<div class="clear row f-12 pb-10 mb-10 bor-b-2">
						<div class="col-sm-4">
							<!-- <div class="sel sel-ck"> 
								<div class="s-input"><span>请选择查询内容</span><i class="icon-caret-down"></i></div>
								<ul class="opt opt-ck line-h-20">
									<li value="userId">用户ID</li>
									<li value="roleName">真实姓名</li>
								</ul>
							</div> -->
						</div>
						<div class="col-sm-4">
							<div class="search">
								<div class="search-con">
									<input id="roleName" name="roleName" type="text" class="ipt-text size-small" placeholder="请输入您所查询的用户姓名" value="">
				...
				<div class="clear mb-10 ">
					<table cellspacing="0" class="bor-primary full-w text-center newline line-h-20 ">
						<caption class="f-16 mb-10">用户·列表</caption>
						...
						<tbody class="text-center line-h-20" id="tradelist">
							<c:forEach var="list" items="${listMap }" varStatus="vs">
								<tr class="<c:if test='${vs.index%2==1 }'> bg-l-green </c:if> <c:if test='${vs.index%2!=1 }'>bg-white</c:if> " resId="${list.id }" resImg="${list.img }">
									...
									<c:if test="${list.user_sex==0 }">
										<td class="pd-5">男</td>
									</c:if>
									<c:if test="${list.user_sex==1 }">
										<td class="pd-5">女</td>
									</c:if>
									<c:if test="${list.user_sex==null }">
										<td class="pd-5"></td>
									</c:if>
									<td class="pd-5">${list.user_mobile }</td>
									<td class="pd-5">${list.user_address }</td>
									<td class="pd-5">${list.user_email }</td>
									<td class="pl-10 pr-10">
										<div class="sel-ck search text-center" id="layer">
											...
											<ul class="opt opt-ck">
												<li class="pd-5 text-center hov-primary cur-p bor-b-l" onclick="javascript:window.location='${pageContext.request.contextPath }/userchangeworker.do?id=${list.id}'">修改</li>
												<li class="pd-5 text-center hov-primary cur-p bor-b-l" onclick="javascript:window.location='${pageContext.request.contextPath }/userdelworker.do?id=${list.id}'">删除</li>
											</ul>
										</div>
									</td>
								</tr>
